...
class DecisionTree():
    def __init__(self, max_depth=4, min_samples_leaf=1, 
                 min_information_gain=0.0, numb_of_features_splitting=None) -> None:
        """
        ...
        Inputs:
            ...
            min_information_gain (float): min information gain required to make the 
                                          splitting possible
            num_of_features_splitting (str):  when splitting 
                                            - if sqrt then sqrt(# of features) features considered, 
    ...
    def _partition_entropy(self, subsets: list) -> float:
        """
        Inputs:
            - subsets (list): list of label lists (EX: [[1,0,0], [1,1,1])
        Returns:
            - Entropy of the 
        """
    ...
    def _select_features_to_use(self, data: np.array) -> list:
        """
        Randomly selects the features to use while splitting with 
        respect to hyperparameter numb_of_features_splitting
        """
